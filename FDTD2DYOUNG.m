...
x = linspace(-0.1,0.2,600);
y = linspace(-0.1,0.2,600);
T = 400;
...
for t = 1:T-1
    ...
    for i = 1:siz
        for j = 1:siz-2
            ...
            if  (i == 100 && ~(j > 240 && j < 250) && ~(j > 150 && j < 160))
                Hx(i, j + 1, t + 1) = 0;
            else
    ...
    for i = 2:siz-2
        for j = 2:siz-2
            ...
            if  i == 3 && (j > 102 && j < siz-302) && t < T/4%((i >  siz - 8 && i < siz - 3 && j <= 199 && j < 201) && t * dt <= 8*(1/(4*f)) ) %((i> round(siz/2) -20 && i < round(siz/2) + 20 && j <= 4) && t < 16)
% 
                Ez(i, j, t + 1) = Ez(i, j , t+1) + planeWave(0,0,0,0,dt,t+1,1,1,f*50,1000,0);
% 
            end
            end
        end

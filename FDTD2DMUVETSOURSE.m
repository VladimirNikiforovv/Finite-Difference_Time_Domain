...
x = linspace(-0.1,0.2,400);
y = linspace(-0.1,0.2,400);
T = 700;
...
for i = 1:siz
    for j = 1:siz
        ...
        cEz(i, j) = (1 - (dt * sigE(i, j)) / (epsilonCUB(yy(i,j)) * eps0 * 2)) / (1 + (dt * sigE(i, j)) / (epsilonCUB(yy(i,j)) * eps0 * 2));
        cHy(i, j) = (dt / (dx * epsilonCUB(yy(i,j)) * eps0)) / (1 + ((dt * sigE(i, j)) / (epsilonCUB(yy(i,j)) * eps0 * 2)));
        cHx(i, j) = (dt / (dy * epsilonCUB(yy(i,j)) * eps0)) / (1 + ((dt * sigE(i, j)) / (epsilonCUB(yy(i,j)) * eps0 * 2)));
    end
end
    ...
    for i = 2:siz-2
        for j = 2:siz-2
            Ez(i, j , t + 1) = cEz(i, j) * Ez(i, j, t) + cHy(i, j ) * (Hy(i+1, j , t + 1 ) - Hy(i, j , t + 1)) - cHx(i, j) * (Hx(i, j+1, t + 1) - Hx(i, j , t + 1));
                ...
                Ez(i, j, t + 1) = Ez(i, j , t+1) + (1/10000)*initcondGauss(x(i),y(j),-0.12 + vs*t,0.05,dt,0.002,0.002)*...
                                                    planeWave(0,0,0,0,dt,t+1,1,1,f*30,1000,0);
